...
# Definición de rangos para el nuevo análisis
SPEED_RANGES = [
...
def process_car_stats(car: Car, car_refuels: List[Refuel]):
    ...
    # Acumuladores para el nuevo análisis por rango
    ...
    for p in periods:
        ...
        for j in range(1, len(p)):
            ...
            if car.has_temporal_data:
                ...
                if hrs > 0:
                    speed = d / hrs
                    for r in SPEED_RANGES:
                        if r["min"] <= speed <= r["max"]:
                            speed_ranges_stats[r["label"]]["km"] += d
                            speed_ranges_stats[r["label"]]["l"] += curr.refuel_litres
                            speed_ranges_stats[r["label"]]["refs"] += 1
                            break
    ...
    if car.has_temporal_data:
        for label, data in speed_ranges_stats.items():
            speed_range_history.append({
                "range_label": label,
                "average_consumption": round(data["l"] / data["km"] * 100, 2) if data["km"] > 0 else 0,
                "number_of_refuels": data["refs"]
            })
...
def display_ui(data: Dict):
    ...
    def print_speed_ranges(history):
        if not history: return
        print(f"  {'Rango de Velocidad':<20} | {'Consumo Medio':<15} | {'Nº Repostajes'}")
        print("  " + "-"*55)
        for r in history:
            print(f"  {r['range_label']:<20} | {r['average_consumption']:<15.2f} | {r['number_of_refuels']}")
    ...
    for car in data["vehicles"]:
        ...
        if "consumption_by_speed_range" in ts and ts["consumption_by_speed_range"]:
            print("\nCONSUMO POR RANGO DE VELOCIDAD:")
            print_speed_ranges(ts["consumption_by_speed_range"])
